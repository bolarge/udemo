...
public class UserRestController {
    ...
    public ResponseEntity<?> createUser(@RequestBody UserCreationRequest userCreationRequest) {
        ...
        User newUser = this.uDemoService.saveUser(userCreationRequest);
        try {
            MailMessage mailMessage = new MailMessage();
            mailMessage.setTo(userCreationRequest.getEmail());
            mailMessage.setSubject("Application Confirmation");
            mailMessage.setText("<html><head></head><body><h3>Dear Applicant</h3><p>Please confirm your email <a href=\"http://localhost:8085/api/user/email/"+newUser.getEmail()+" \">to complete your application</a>.</p></body></html>");
        ...
        URI newPollUri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(newUser.getId()).toUri();
    ...
    public ResponseEntity<?> updateUser(@PathVariable Integer userId, @RequestBody User user) {
        verifyUser(userId);
        User updateUser = this.uDemoService.getUserRepository().save(user);
    ...
    public ResponseEntity<?> deleteUser(@PathVariable Integer userId) {
        ...
        this.uDemoService.getUserRepository().save(userToBeDeleted.get());
    ...
    public ResponseEntity<?> verifyUser(@PathVariable("email") String email) throws Exception {
        ...
        this.uDemoService.getUserRepository().save(verifiedUser);
